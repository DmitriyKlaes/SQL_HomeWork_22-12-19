...
SELECT c.`id` AS id_cat, c.`name`, s.`shopname`, s.`id` AS id_shop 
FROM `cats` c 
LEFT JOIN `shops` s 
ON s.id = c.shops_id;

-- 2. Вывести магазин, в котором продается кот “Мурзик” (попробуйте выполнить 2 способами)
-- 2.a) С помощью join-a
SELECT s.`shopname` 
FROM `shops` s 
LEFT JOIN `cats` c 
ON s.id = c.shops_id 
...
-- 2.b) С помощью подзапроса
SELECT s.`shopname` 
FROM `shops` s 
WHERE s.`id` IN (SELECT c.`id` FROM `cats` c WHERE c.`name` = "Murzik");

-- 2.c) Без подзапроса и join-a
SELECT s.`shopname` 
FROM `shops` s, `cats` c 
...
SELECT s.`shopname` 
FROM `shops` s 
WHERE s.`id` NOT IN (SELECT `shops_id` FROM `cats` WHERE `name` IN ("Murzik", "Zuza"));
...
FROM `Analysis` a 
RIGHT JOIN (SELECT * FROM `Orders` WHERE ord_datetime BETWEEN "2020-02-05 00:00:00" AND "2020-02-12 23:59:59") o 
ON a.`an_id` = o.`ord_an`;
